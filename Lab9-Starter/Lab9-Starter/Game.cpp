...
/// save result in m_showFace
/// </summary>
/// <param name="t_index">No of Face 0-5</param>
/// <returns>true if pointing towards camera</returns>
bool Game::checkFace(const int t_index) // checks the faces
{
	MyVector3 cross = (m_points[m_cubeVertexs[t_index][2]] - m_points[m_cubeVertexs[t_index][0]]);
	m_showFace[t_index] = false;

	if ((m_points[m_cubeVertexs[t_index][3]] - m_points[m_cubeVertexs[t_index][0]]).crossProduct(cross).z > 0) // if it is pointing towards the camera(me) then it is true or else its false
	{
